...
class GB_CCA_HV(nn.Module):
    def __init__(self, dim, num_heads, bias=False):
        super(GB_CCA_HV, self).__init__()
    ...
    def forward(self, x, y,illum_map):
        weights=self.weight_predictor(illum_map)
        illum_feat=self.map_proj(illum_map)
        w_x, w_y = weights[:, 0:1], weights[:, 1:2]

        y = y + w_y * (y * torch.sigmoid(illum_feat))
        x = x + w_x * (x * torch.sigmoid(illum_feat))
...
class GB_CCA_I(nn.Module):
    def __init__(self, dim, num_heads, bias=False):
        super(GB_CCA_I, self).__init__()
        self.norm = LayerNorm(dim)
        self.gdfn = IEL(dim)
